...
def stratify_text_cl(corpus, perc: float, skip_header: bool = True, train_path=None, eval_path=None):
  ...
  with open(corpus, 'r') as corpus_file:
    ...
    if skip_header:
      ...
      for line in csvreader:
        if line[1] != '':
          classes_dev_count_pair = [(pair, idx) for idx, pair in enumerate(classes_dev_count) if pair[0] == line[1]][0]
          if classes_dev_count_pair[0][1] > 0:
            eval_file.write(f'"{line[0]}",{line[1]}\n')
            ...
            train_file.write(f'"{line[0]}",{line[1]}\n')
